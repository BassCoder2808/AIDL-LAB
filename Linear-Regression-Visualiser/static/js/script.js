var svg = d3.select("svg").attr("width", 650).attr("height", 650);
var margin = {
  top: 25,
  bottom: 25,
  left: 25,
...
g.append("g")
  ...
  .call(d3.axisBottom().scale(xScale).ticks(4));
g.append("g")
  .attr("class", "y-axis")
  .call(d3.axisLeft().scale(yScale).ticks(4));
...
function render_plot(dataset, estimates) {
  ...
  if (dataset.length >= 2) {
    ...
    var line_dat = plot_x_vals.map((d) => ({
      x: d,
      y: estimates.intercept + estimates.slope * d,
    }));
  }

  if (dataset.length >= 2 && !d3.selectAll(".regline").empty()) {
    d3.selectAll(".regline").datum(line_dat).transition().attr("d", line);
  } else if (dataset.length >= 2 && d3.selectAll(".regline").empty()) {
    g.append("path").datum(line_dat).attr("class", "regline").attr("d", line);
  }
}
...
document.getElementById("button").onclick = function (event) {
  d3.request("/data").send("DELETE", function (d) {
    d3.json(
      "/model",
      (callback = function (estimates) {
